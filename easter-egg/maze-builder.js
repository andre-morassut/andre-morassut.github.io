...
/*
  ...
  - exported class MazeBuilder
  - added methods to place goodies & monsters : placeMonstersAndBonuses
*/
...
export class MazeBuilder {
    ...
    placeItems() {
      let nbBonus = Math.max(1, Math.round((this.cols - 7) / 2));
      let bonuses = ["treasure", "coin", "gem"] ;
      while (nbBonus > 0) {
        let i = Math.round(Math.random() * (this.rows - 1));
        let j = Math.round(Math.random() * (this.cols - 1));
        if (this.maze[i][j].length == 0) {
          this.maze[i][j] = bonuses[Math.floor(Math.random() * bonuses.length)];
          nbBonus--;
        }
      }
      let nbMalus = Math.round((this.cols - 7) / 2) + (this.cols > 11 ? this.cols - 11 : 0);
      let maluses = ["bomb", "spiky"];
      while (nbMalus > 0) {
        let i = Math.round(Math.random() * (this.rows - 1));
        let j = Math.round(Math.random() * (this.cols - 1));
        if (this.maze[i][j].length == 0) {
          this.maze[i][j] = maluses[Math.floor(Math.random() * maluses.length)];
          nbMalus--;
        }
      }
      let stars = Math.round(Math.random() * 3);
      while (stars == 0) {
        let i = Math.round(Math.random() * (this.rows - 1));
        let j = Math.round(Math.random() * (this.cols - 1));
        if (this.maze[i][j].length == 0) {
      ...
      let mushroom = Math.random() < 0.5 ? 0 : 1;
      while (mushroom == 0) {
        let i = Math.round(Math.random() * (this.rows - 1));
        let j = Math.round(Math.random() * (this.cols - 1));
        if (this.maze[i][j].length == 0) {
          this.maze[i][j] = ["mushroom"];
          mushroom--;
        }
      }
    }
