...
// const textLinesReveal = new TextReveal([DOM.textContent.primary, DOM.textContent.secondary]);
const textPresReveal = new _textReveal.TextReveal([
    DOM.textContent.primary
]);
const textLinesReveal2 = new _textLinesReveal.TextLinesReveal([
...
class Menu {
    constructor(el){
        this.DOM = {
            ...
            detailLinks: document.querySelectorAll('.details__content-link'),
            ...
            circle: document.getElementById('circles')
        };
    ...
    open() {
        ...
        this.textReveal.in();
        this.DOM.circle.classList.add('circles-hidden');
    }
    ...
    // close the menu (animate the menu items out) - without showing the circles
    close() {
        this.textReveal.out().then(()=>this.DOM.el.classList.remove('menu--open')
        );
        this.DOM.circle.classList.remove('circles-hidden');
    }
    initEvents() {
        ...
        // clicking a detail link will open the corresponding section
        this.DOM.detailLinks.forEach((detailLinks, pos)=>{
            detailLinks.addEventListener('click', (ev)=>{
                ev.preventDefault();
                this.openInnerDetails(pos + 1);
            });
        });
    ...
    openInnerDetails(pos) {
        if (this.menuItemCurrent === -1) return;
        this.detailsInstances[this.menuItemCurrent].closeForInnerTransition();
        // reset
        this.menuItemCurrent = pos;
        //this.detailsInstances[this.menuItemCurrent].open();
        let currentDetail = this.detailsInstances[this.menuItemCurrent];
        setTimeout(()=>currentDetail.open()
        , 300);
    /*
        // save position
        this.menuItemCurrent = pos;
...
class Details {
    constructor(el, closeDetailsCtrl){
        this.DOM = {
            ...
            texts: el.querySelectorAll('.details__content > .details__content-text'),
            rtf: el.querySelector('.details__content > .details__content-rtf'),
            link: el.querySelector('.details__content > .details__content-link'),
            closeDetailsCtrl: closeDetailsCtrl
        };
        this.textLinesR = new _textLinesReveal.TextLinesReveal(this.DOM.texts);
        // TextReveal obj (this.DOM.title and this.DOM.link animation)
        let textRevealElts = [
            this.DOM.title,
            this.DOM.closeDetailsCtrl
        ];
        if (this.DOM.rtf !== null) textRevealElts.push(this.DOM.rtf);
        if (this.DOM.link !== null) textRevealElts.push(this.DOM.link);
        this.textReveal = new _textReveal.TextReveal(textRevealElts);
    }
    open() {
...
class TextLinesReveal {
    constructor(animationElems){
        ...
        for (const el of this.DOM.animationElems){
            if (NodeList.prototype.isPrototypeOf(el)) for (const subEl of el){
                const SplitTypeInstance = new _splitTypeDefault.default(subEl, {
                    types: 'lines'
                });
...
},{"split-type":"6KwHJ","./utils":"fIYUT","gsap":"2aTR0","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}]},["cSv3F","3auaO"], "3auaO", "parcelRequired7cf")

//# sourceMappingURL=index-fr.8b7fb9b3.js.map
